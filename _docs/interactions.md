...
Only the current Sales owner or an Admin may create records. Closed customers remain readable, but they must be reopened before a new interaction or note is created. Each save has a submission ID, so a retry or double submission returns the original record; separate submission IDs create separate records.

History deletion (#160): the customer's current Sales owner or an Admin may delete **any** history entry on that customer, including entries written by other users (for example a previous owner), and including entries on Closed customers. Other Sales users get 403. Deletion is a soft tombstone (`deleted: true`, `deletedBy`, `deletedAt`) with a `History deleted` audit row; deleting an entry again returns the same tombstone.
