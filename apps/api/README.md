...
Every request, DB query, and the existing request log are instrumented with OpenTelemetry (`otel_setup.py`), exporting over OTLP only when `OTEL_EXPORTER_OTLP_ENDPOINT` is set -- unset (the default here and in CI) means it's a complete no-op, no exporter, no network call. See the "OpenTelemetry" section of [`../../_docs/deployment.md`](../../_docs/deployment.md) for the environment variables and [`../../_docs/grafana-dashboard.json`](../../_docs/grafana-dashboard.json) for the dashboard model.
