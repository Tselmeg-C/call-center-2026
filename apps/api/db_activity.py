...
from .db_assignment import AssignmentHistoryRow, AuditRow
...
class ActivityDatabase:
    ...
    def followups_map(self, bcns: list[str]) -> dict[str, list[dict]]:
        """Same batched-by-bcn shape as history_map, for the /customers list endpoint to embed
        each row's follow-ups without one query per row (#118)."""
        if not bcns: return {}
        with Session(self.engine) as session:
            rows = session.scalars(select(FollowUpRow).where(FollowUpRow.bcn.in_(bcns)).order_by(FollowUpRow.created_at, FollowUpRow.id))
            result: dict[str, list[dict]] = {}
            for item in rows:
                result.setdefault(item.bcn, []).append({"id": item.id, "bcn": item.bcn, "type": item.type, "due": item.due.isoformat() if item.due else None, "note": item.note, "status": item.status, "interactionId": item.interaction_id, "actorId": item.actor_id, "createdAt": item.created_at.isoformat(), "updatedAt": item.updated_at.isoformat()})
            return result
    ...
    def reopen_customer(self, bcn: str, event: dict, *, owner_id: str | None, status: str, version: int, actor_id: str, submission_id: str, payload: str, result: dict, released_owner_id: str | None = None) -> None:
        """Commit the customer's owner/status/version correction, the reopen event, the retry
        record, and (when released_owner_id is given) the ownership-release assignment_history/
        audit_events rows, all in the same transaction (same physical database as customer_db and
        assignment_db, via this engine) -- never via assignment_db.append_assignment/append_audit,
        which each open and commit their own separate session."""
        with Session(self.engine) as session:
            ...
            if released_owner_id is not None:
                released_at = datetime.now(timezone.utc)
                session.add(AssignmentHistoryRow(bcn=bcn, actor_id=actor_id, old_owner_id=released_owner_id, new_owner_id=None, reason="Owner no longer active Sales", created_at=released_at))
                session.add(AuditRow(actor_id=actor_id, action="Customer ownership released", target=bcn, details={"oldOwner": released_owner_id, "newOwner": None, "reason": "Owner no longer active Sales"}, created_at=released_at))
