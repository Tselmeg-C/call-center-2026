...
function AssignmentView() { const { services } = useServices(); const [scope, setScope] = useState<"unassigned" | "all">("unassigned"); const [result, setResult] = useState<Result<import("../../services/types").AssignmentRunResult> | null>(null); const [error, setError] = useState(""); const [pending, setPending] = useState(false); async function run(event: FormEvent) { event.preventDefault(); if (pending) return; setPending(true); setError(""); const value = await services.runAssignments({ scope, submissionId: `run-${scope}` }); setPending(false); if (!value.ok) setError(value.error.message); else setResult(value); } return <section aria-label="Assignment settings"><p>Placeholder — assignment rules are synthetic.</p><form onSubmit={run}><label>Scope <select value={scope} onChange={e => setScope(e.target.value as "unassigned" | "all")}><option value="unassigned">Unassigned open customers</option><option value="all">All open customers</option></select></label><button type="submit" disabled={pending}>{pending ? "Running…" : "Run assignment"}</button></form>{error && <p role="alert">{error}</p>}{result?.ok && <><p>Scanned {result.data.scanned}; assigned {result.data.assigned}; reassigned {result.data.reassigned}; unchanged {result.data.unchanged}; skipped {result.data.skipped}</p><ul>{result.data.results.map(item => <li key={item.bcn}>{item.bcn}: {item.oldOwner ?? "Unassigned"} → {item.newOwner ?? "Unassigned"} ({item.reason})</li>)}</ul></>}</section>; }
