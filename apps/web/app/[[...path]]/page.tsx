...
import { workloadBuckets, type AssignmentRule, type ClosureReason, type Customer, type CustomerDetail, type FollowUpType, type ImportResult, type Result, type SampleRecord, type Scenario, type User, type WorkloadBucket, type WorkloadCustomer, type WorkloadData } from "../../services/types";
...
function AssignmentView() {
  const { services } = useServices(); const [scope, setScope] = useState<"unassigned" | "all">("unassigned"); const [rules, setRules] = useState<Result<AssignmentRule[]> | null>(null); const [sales, setSales] = useState<User[]>([]); const [result, setResult] = useState<Result<import("../../services/types").AssignmentRunResult> | null>(null); const [error, setError] = useState(""); const [pending, setPending] = useState(false); const [name, setName] = useState(""); const [value, setValue] = useState("A"); const [eligible, setEligible] = useState<string[]>(["sales-river"]); const [fallback, setFallback] = useState<string[]>([]);
  const load = () => { void services.assignmentRules().then(setRules); void services.listUsers().then(r => { if (r.ok) setSales(r.data.filter(u => u.role === "Sales")); }); void services.assignmentFallback().then(r => { if (r.ok) setFallback(r.data); }); }; useEffect(load, [services]);
  async function run(event: FormEvent) { event.preventDefault(); if (pending) return; setPending(true); setError(""); const r = await services.runAssignments({ scope, submissionId: `run-${scope}` }); setPending(false); if (!r.ok) setError(r.error.message); else setResult(r); }
  async function create(event: FormEvent) { event.preventDefault(); setError(""); const r = await services.createAssignmentRule({ name, active: true, field: "propensityTier", operator: "=", value, eligibleSalesIds: eligible }); if (!r.ok) setError(r.error.message); else { setName(""); load(); } }
  async function saveFallback(event: FormEvent) { event.preventDefault(); const r = await services.setAssignmentFallback(fallback); if (!r.ok) setError(r.error.message); else load(); } async function toggle(rule: AssignmentRule) { const r = await services.updateAssignmentRule(rule.id, { active: !rule.active }); if (!r.ok) setError(r.error.message); else load(); } async function move(rule: AssignmentRule, delta: number) { if (!rules?.ok) return; const other = rules.data.find(item => item.position === rule.position + delta); if (!other) return; const first = await services.updateAssignmentRule(rule.id, { position: other.position }); if (!first.ok) { setError(first.error.message); return; } const second = await services.updateAssignmentRule(other.id, { position: rule.position }); if (!second.ok) setError(second.error.message); else load(); }
  return <section aria-label="Assignment settings"><p>Placeholder — assignment rules and balancing are synthetic.</p><form onSubmit={create}><label>Rule name <input value={name} onChange={e => setName(e.target.value)} /></label><label>Tier <select value={value} onChange={e => setValue(e.target.value)}><option>A</option><option>B</option></select></label><label>Eligible Sales <select multiple value={eligible} onChange={e => setEligible([...e.target.selectedOptions].map(o => o.value))}>{sales.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select></label><button type="submit">Create rule</button></form><form onSubmit={saveFallback}><label>Fallback Sales <select multiple value={fallback} onChange={e => setFallback([...e.target.selectedOptions].map(o => o.value))}>{sales.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select></label><button type="submit">Save fallback</button></form><form onSubmit={run}><label>Scope <select value={scope} onChange={e => setScope(e.target.value as "unassigned" | "all")}><option value="unassigned">Unassigned open customers</option><option value="all">All open customers</option></select></label><button type="submit" disabled={pending}>{pending ? "Running…" : "Run assignment"}</button></form><h2>Rules</h2>{rules?.ok && <ul>{rules.data.map(rule => <li key={rule.id}>{rule.position}. {rule.name} · {rule.field} {rule.operator} {rule.value} · {rule.active ? "Active" : "Inactive"} <button type="button" onClick={() => void toggle(rule)}>{rule.active ? "Deactivate" : "Activate"}</button> <button type="button" onClick={() => void move(rule, -1)} disabled={rule.position === 1}>Up</button> <button type="button" onClick={() => void move(rule, 1)} disabled={rule.position === rules.data.length}>Down</button></li>)}</ul>}{error && <p role="alert">{error}</p>}{result?.ok && <><p>Scanned {result.data.scanned}; assigned {result.data.assigned}; reassigned {result.data.reassigned}; unchanged {result.data.unchanged}; skipped {result.data.skipped}</p><ul>{result.data.results.map(item => <li key={item.bcn}>{item.bcn}: {item.oldOwner ?? "Unassigned"} → {item.newOwner ?? "Unassigned"} ({item.reason})</li>)}</ul></>}</section>;
}
...
function CustomerDetailView({ bcn }: { bcn: string }) {
  const { services, user } = useServices(); const [result, setResult] = useState<Result<CustomerDetail> | null>(null); const [owners, setOwners] = useState<User[]>([]); const [assignmentOwner, setAssignmentOwner] = useState("");
  ...
  useEffect(() => { if (user?.role === "Admin") void services.listUsers().then(value => { if (value.ok) setOwners(value.data.filter(item => item.role === "Sales" && item.active !== false)); }); }, [services, user?.role]);
  ...
  return <><Link href={`/customers${listQuery}`}>← Back to customers</Link><h1>{c.name}</h1><p><b>BCN:</b> {c.bcn} · <b>Owner:</b> {c.ownerName ?? "Unassigned"} · <b>Status:</b> {c.status}</p><p><b>Contact status:</b> {c.contactStatus} · <b>Next action:</b> {dash(c.nextFollowUp)} (UTC)</p><p>{readOnly ? "Read-only for your role." : user?.role === "Admin" ? "Admin can manage this customer." : "You can work on this customer."}</p>
    ...
    <section aria-label="Assignment"><h2>Assignment</h2>{user?.role === "Admin" && <form onSubmit={async event => { event.preventDefault(); const value = await services.assignCustomer(bcn, assignmentOwner || null, `manual-${bcn}`); if (!value.ok) setError(value.error.message); else load(); }}><select aria-label="Owner" value={assignmentOwner || c.ownerId || ""} onChange={event => setAssignmentOwner(event.target.value)}><option value="">Unassigned</option>{owners.map(owner => <option key={owner.id} value={owner.id}>{owner.name}</option>)}</select><button type="submit">Save assignment</button></form>}</section><h2>Imported customer source information</h2><SourceGroups source={c.source} /><h2>Phones</h2><ul>{c.phones.length ? c.phones.map(phone => <li key={phone}>{phone}</li>) : <li>—</li>}</ul><h2>History</h2><p>{c.histories.length} records</p><ul>{history.map(item => <li key={item.id}>{item.deleted ? `${item.timestamp} · ${item.deletedBy} · Deleted ${item.kind} (recorded ${item.deletedAt})` : <>{item.timestamp} · {item.actor} · {item.kind}: {item.text}{item.outcome ? ` (${item.outcome})` : ""}{item.interactionId ? ` (interaction ${item.interactionId})` : ""}</>}{canDelete && !item.deleted && (item.kind === "Interaction" || item.kind === "Standalone note") && <button type="button" onClick={() => void remove(item)} disabled={pending !== null}>Delete</button>}</li>)}</ul><p><button disabled={historyPage === 1} onClick={() => setHistoryPage(historyPage - 1)}>Previous history</button> Page {historyPage} of {historyPages} <button disabled={historyPage === historyPages} onClick={() => setHistoryPage(historyPage + 1)}>Next history</button></p><SamplePanel /></>;
}
  ...
  return <main>
    ...
    {redirect ? <p role="status">Returning to {user ? "your workspace" : "sign-in"}…</p> : login ? <SignIn key={snapshot.notice + snapshot.reset} /> : !route && !customerRoute && !detailBcn ? <>
      ...
      {detailBcn ? <CustomerDetailView bcn={detailBcn} /> : customerRoute ? <><h1>{pathname === "/customers/mine" ? "My Customers" : "All Customers"}</h1><Customers mine={pathname === "/customers/mine"} /></> : route![0] === "/dashboard" ? <><h1>Dashboard</h1><Dashboard /></> : route![2] === "Admin" && user.role !== "Admin" ? <><h1>Access denied</h1><p>This page requires the Admin role.</p><Link href="/dashboard">Back to Dashboard</Link></> : <>
        <h1>{route![1]}</h1>{pathname === "/admin/users" ? <AdminSettings reasons={false} /> : pathname === "/admin/closure-reasons" ? <AdminSettings reasons /> : pathname === "/admin/import" ? <ImportView /> : pathname === "/admin/assignments" ? <AssignmentView /> : <p>Placeholder — this functionality is not implemented yet.</p>}
