...
function AssignmentView() {
  const { services } = useServices(); const [scope, setScope] = useState<"unassigned" | "all">("unassigned"); const [rules, setRules] = useState<Result<AssignmentRule[]> | null>(null); const [sales, setSales] = useState<User[]>([]); const [result, setResult] = useState<Result<import("../../services/types").AssignmentRunResult> | null>(null); const [error, setError] = useState(""); const [pending, setPending] = useState(false); const [name, setName] = useState(""); const [value, setValue] = useState("A"); const [eligible, setEligible] = useState<string[]>(["sales-river"]);
  const load = () => { void services.assignmentRules().then(setRules); void services.listUsers().then(r => { if (r.ok) setSales(r.data.filter(u => u.role === "Sales")); }); }; useEffect(load, [services]);
