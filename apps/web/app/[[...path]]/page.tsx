...
import type { Customer, CustomerDetail, FollowUpType, Result, SampleRecord, Scenario, User } from "../../services/types";
...
function CustomerDetailView({ bcn }: { bcn: string }) {
  ...
  async function complete(item: typeof c.followUps[number]) { if (pending) return; const value = await services.completeFollowUp({ bcn, followUpId: item.id, outcome: "Contact", submissionId: submissionId("complete") }); if (!value.ok) setError(value.error.message); else load(); }
  async function close() { if (pending || !closureReason) return; setPending("close"); const value = await services.closeCustomer({ bcn, reasonId: closureReason, submissionId: submissionId("close") }); setPending(null); if (!value.ok) setError(value.error.message); else load(); }
