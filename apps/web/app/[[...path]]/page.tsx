...
function Customers({ mine }: { mine: boolean }) {
  ...
  useEffect(() => { const params = new URLSearchParams(); if (query) params.set("q", query); if (status !== "all") params.set("status", status); if (!mine && owner !== "all") params.set("owner", owner); if (contacted !== "all") params.set("contacted", contacted); if (recent !== "all") params.set("recent", recent); if (tier !== "all") params.set("tier", tier); if (sort !== "bcn") params.set("sort", sort); if (descending) params.set("dir", "desc"); if (page !== 1) params.set("page", String(page)); if (pageSize !== 25) params.set("size", String(pageSize)); const suffix = params.toString(); window.history.replaceState(null, "", `${window.location.pathname}${suffix ? `?${suffix}` : ""}`); }, [query, status, owner, contacted, recent, tier, sort, descending, page, pageSize, mine]);
