...
function AssignmentView() { const { services } = useServices(); const [scope, setScope] = useState<"unassigned" | "all">("unassigned"); const [rules, setRules] = useState<Result<AssignmentRule[]> | null>(null); const [result, setResult] = useState<Result<import("../../services/types").AssignmentRunResult> | null>(null); const [error, setError] = useState(""); const [pending, setPending] = useState(false); const load = () => void services.assignmentRules().then(setRules); useEffect(load, [services]); async function run(event: FormEvent) { event.preventDefault(); if (pending) return; setPending(true); setError(""); const value = await services.runAssignments({ scope, submissionId: `run-${scope}` }); setPending(false); if (!value.ok) setError(value.error.message); else setResult(value); } async function toggle(rule: AssignmentRule) { const value = await services.updateAssignmentRule(rule.id, { active: !rule.active }); if (!value.ok) setError(value.error.message); else load(); } return <section aria-label="Assignment settings"><p>Placeholder — assignment rules are synthetic.</p><form onSubmit={run}><label>Scope <select value={scope} onChange={e => setScope(e.target.value as "unassigned" | "all")}><option value="unassigned">Unassigned open customers</option><option value="all">All open customers</option></select></label><button type="submit" disabled={pending}>{pending ? "Running…" : "Run assignment"}</button></form><h2>Rules</h2>{rules?.ok && <ul>{rules.data.map(rule => <li key={rule.id}>{rule.position}. {rule.name} · {rule.field} {rule.operator} {rule.value} · {rule.active ? "Active" : "Inactive"} <button type="button" onClick={() => void toggle(rule)}>{rule.active ? "Deactivate" : "Activate"}</button></li>)}</ul>}{error && <p role="alert">{error}</p>}{result?.ok && <><p>Scanned {result.data.scanned}; assigned {result.data.assigned}; reassigned {result.data.reassigned}; unchanged {result.data.unchanged}; skipped {result.data.skipped}</p><ul>{result.data.results.map(item => <li key={item.bcn}>{item.bcn}: {item.oldOwner ?? "Unassigned"} → {item.newOwner ?? "Unassigned"} ({item.reason})</li>)}</ul></>}</section>; }
