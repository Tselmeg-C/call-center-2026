...
function CustomerDetailView({ bcn }: { bcn: string }) {
  ...
  const listQuery = typeof window === "undefined" ? "" : window.location.search;
  return <><Link href={`/customers${listQuery}`}>← Back to customers</Link><h1>{c.name}</h1><p><b>BCN:</b> {c.bcn} · <b>Owner:</b> {c.ownerName ?? "Unassigned"} · <b>Status:</b> {c.status}</p><p><b>Contact status:</b> {c.contactStatus} · <b>Next action:</b> {dash(c.nextFollowUp)}</p><p>{readOnly ? "Read-only for your role." : user?.role === "Admin" ? "Admin can manage this customer." : "You can work on this customer."}</p><h2>Imported customer source information</h2><SourceGroups source={c.source} /><h2>Phones</h2><ul>{c.phones.length ? c.phones.map(phone => <li key={phone}>{phone}</li>) : <li>—</li>}</ul><h2>History</h2><p>{c.histories.length} records</p><ul>{history.map(item => <li key={item.id}>{item.timestamp} · {item.actor} · {item.kind}: {item.text}</li>)}</ul><p><button disabled={historyPage === 1} onClick={() => setHistoryPage(historyPage - 1)}>Previous history</button> Page {historyPage} of {historyPages} <button disabled={historyPage === historyPages} onClick={() => setHistoryPage(historyPage + 1)}>Next history</button></p><SamplePanel /></>;
}
