import type { AssignmentRule, AuditEvent, AuditQuery, ClosureReason, CreateInteractionInput, CreateNoteInput, Customer, CustomerDetail, FollowUp, FollowUpInput, HistoryRecord, ImportResult, MockControls, MockSnapshot, ReportData, Result, SampleRecord, Services, User, UpdateFollowUpInput, CompleteFollowUpInput, LifecycleInput } from "./types";
...
export function createMockAdapter(options: { now?: () => string } = {}): { services: Services; controls: MockControls } {
  ...
  const services: Services = {
    ...
    audit: (query: AuditQuery = {}) => { if (!user || user.role !== "Admin") return Promise.resolve(failure("forbidden", "Admin access required.")); const events: AuditEvent[] = [...auditEvents, ...customers.flatMap(c => c.histories.filter(h => ["Assignment", "Closure", "Reopen", "Interaction", "Standalone note"].includes(h.kind)).map(h => ({ id: h.id, actor: h.actor, actorId: h.actorId ?? "", action: h.kind, target: c.bcn, timestamp: h.timestamp, details: { text: h.deleted ? null : h.text } })))] .filter(event => (!query.actor || event.actorId === query.actor || event.actor.toLowerCase().includes(query.actor.toLowerCase())) && (!query.action || event.action === query.action) && (!query.bcn || event.target === query.bcn) && (!query.start || event.timestamp.slice(0, 10) >= query.start) && (!query.end || event.timestamp.slice(0, 10) <= query.end)).sort((a, b) => b.timestamp.localeCompare(a.timestamp) || b.id.localeCompare(a.id)); return Promise.resolve({ ok: true, data: events }); },
