...
export function createMockAdapter(): { services: Services; controls: MockControls } {
  ...
  const services: Services = {
    ...
    getCustomer: bcn => {
      if (!user) return Promise.resolve(failure("unauthenticated", "Sign in to continue."));
      return request(() => { const customer = customerFixtures.find(item => item.bcn === bcn); return customer ? { ok: true, data: { ...customer, histories: [...customer.histories].sort((a, b) => b.timestamp.localeCompare(a.timestamp) || b.id.localeCompare(a.id)) } } : failure("request-failure", "Customer not found."); });
    },
