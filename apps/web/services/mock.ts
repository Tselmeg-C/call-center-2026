...
export function createMockAdapter(options: { now?: () => string } = {}): { services: Services; controls: MockControls } {
  ...
  function createInteraction(input: CreateInteractionInput): Result<HistoryRecord> {
    ...
    const record: HistoryRecord = { id: `interaction-${nextRecord++}`, kind: "Interaction", actor: user!.name, actorId: user!.id, timestamp: now(), text: text.data || null, outcome: input.outcome };
  ...
  function deleteHistory(bcn: string, recordId: string): Result<HistoryRecord> {
    ...
    record.deleted = true; record.deletedAt = now(); record.deletedBy = user.name; record.deletedById = user.id; record.text = null; delete record.outcome;
