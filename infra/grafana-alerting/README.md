...
| --- | --- | --- |
| `alert-rule-health-ready.json` | Alert rule: `/health/ready` failing on `development`, sourced from a Grafana Synthetic Monitoring HTTP check's `probe_success` metric. Does not depend on OTel export. `noDataState: OK` since #99 (a stopped check is the stale-monitor alert's job). | **Applied and live.** |
...
| `alert-rule-health-monitor-stale.json` | Dead man's switch (#99): fires when `sum(count_over_time(probe_success{job="health-ready-development"}[30m]))` has no data, i.e. the development SM check recorded nothing for 30 minutes. Labelled `kind=monitor-stale`, no `team=on-call`. | **Applied and live** (2026-09-24, `Normal`). |
| `notification-policy-route-monitor-stale.json` | Route listed **first**: matches `kind=monitor-stale` and sends to `TselmegC` with `continue: false`, so the stale alert emails once and never reaches the GitHub-issue route. | **Applied and live** (2026-09-24, #99). |
