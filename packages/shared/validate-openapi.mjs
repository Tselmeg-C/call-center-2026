...
for (const path of ["/session/login", "/session/logout", "/session/me", "/customers", "/customers/{bcn}", "/workload", "/customers/{bcn}/follow-ups", "/customers/{bcn}/follow-ups/{followUpId}", "/customers/{bcn}/follow-ups/{followUpId}/cancel", "/customers/{bcn}/follow-ups/{followUpId}/complete", "/admin/users", "/admin/closure-reasons", "/admin/imports", "/admin/assignment-rules", "/admin/assignment-rules/{id}", "/admin/assignments", "/admin/assignments/run", "/admin/reports", "/admin/audit"]) assert.match(text, new RegExp(`^  ${path.replace(/[{}]/g, "\\$&")}:`, "m"));
