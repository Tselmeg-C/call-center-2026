...
for (const path of ["/session/login", "/session/logout", "/session/me", "/customers", "/customers/{bcn}", "/workload", "/customers/{bcn}/follow-ups", "/customers/{bcn}/follow-ups/{followUpId}", "/customers/{bcn}/follow-ups/{followUpId}/complete"]) assert.match(text, new RegExp(`^  ${path.replace(/[{}]/g, "\\$&")}:`, "m"));
